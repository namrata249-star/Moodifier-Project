...
public class ChangeMoodActivity extends AppCompatActivity {
    ...
    private List<NewsFeed> albumList,aList;
...
// Navigate to SuggestionsActivity after the mood logic
        Intent intent = new Intent(ChangeMoodActivity.this, SuggestionsActivity.class);
        intent.putExtra("MOOD", mood); // Pass the selected mood to SuggestionsActivity
        ...
        adapter = new NewsFeedAdapter(this, aList,recyclerView);
            ...
            public void onLoadMore() {
                ...
                handler.postDelayed(new Runnable() {
                    ...
                    public void run() {
                        ...
                        adapter.notifyItemRemoved(aList.size());
                        //add items one by one
                        int start = aList.size();
                        int end = start + 5;

                        for (int i = start + 1; i <= end && i<albumList.size(); i++) {
                            aList.add(albumList.get(i));
                            adapter.notifyItemInserted(aList.size());
                        }
                        adapter.setLoaded();
